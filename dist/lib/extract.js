...
const monthYearRegex = new RegExp(`\\b(?:${monthNames.join("|")})\\s+(20\\d{2}|19\\d{2})\\b`, "gi");
...
export function extractFromSnippet(snippet, base) {
...
export function aggregateSeries(points) {
    ...
    const key = (p) => `${p.date}|${p.price}|${p.sourceUrl}`;
    const map = new Map();
...
export function buildTrend(points) {
